...
import { installLaunchRoutes, authRateLimit } from "./lib/launch.js";
import { syncConnect } from "./lib/connect.js";
...
import { SERVICE_TYPES, validateProfile, isAvailable, activeMembership, milesBetween } from "./lib/marketplace.js";

const app=express(), port=Number(process.env.PORT||4242), baseUrl=((process.env.VERCEL_ENV==="preview" && process.env.VERCEL_URL ? "https://"+process.env.VERCEL_URL : process.env.BASE_URL)||(process.env.VERCEL?"https://www.maybridgecarecollective.com":`http://localhost:${port}`)).replace(/\/$/,"");
const stripe=process.env.STRIPE_SECRET_KEY ? new Stripe(process.env.STRIPE_SECRET_KEY,{maxNetworkRetries:2,timeout:15000}) : null;
...
const requiredStripe=()=>{if(!stripe)throw fail(503,"Payment setup is not available yet.");if(/^(sk|rk)_live_/.test(process.env.STRIPE_SECRET_KEY)&&process.env.LIVE_PAYMENTS_ENABLED!=="true")throw fail(503,"Live payments are not enabled yet.");return stripe};
...
app.post("/api/care-recipients",auth("customer"),async(req,res,next)=>{try{const name=String(req.body.preferred_name||"").trim();if(!name||name.length>150)throw fail(400,"Preferred name is required (up to 150 characters).");const {rows}=await db().query("INSERT INTO care_recipients(customer_id,preferred_name,relationship,notes) VALUES($1,$2,$3,$4) RETURNING id,preferred_name,relationship,notes,created_at",[req.user.id,name,String(req.body.relationship||"").slice(0,100),String(req.body.notes||"").slice(0,4000)]);res.status(201).json({care_recipient:rows[0]})}catch(e){next(e)}});
app.get("/api/care-recipients",auth("customer"),async(req,res,next)=>{try{const {rows}=await db().query("SELECT id,preferred_name,relationship,notes,created_at FROM care_recipients WHERE customer_id=$1 ORDER BY created_at",[req.user.id]);res.json({care_recipients:rows})}catch(e){next(e)}});
app.put("/api/provider/profile",auth("provider"),async(req,res,next)=>{try{const b=validateProfile(req.body),{rows}=await db().query(`UPDATE provider_profiles SET headline=$2,bio=$3,experience_years=$4,base_zip=$5,service_radius_miles=$6,service_zips=$7,service_types=$8,qualifications=$9,languages=$10,certifications=$11,education=$12,work_history=$13,availability=$14,admin_approval_status=CASE WHEN headline IS DISTINCT FROM $2 OR bio IS DISTINCT FROM $3 OR experience_years IS DISTINCT FROM $4 OR base_zip IS DISTINCT FROM $5 OR service_radius_miles IS DISTINCT FROM $6 OR service_zips IS DISTINCT FROM $7::text[] OR service_types IS DISTINCT FROM $8::text[] OR qualifications IS DISTINCT FROM $9::text[] OR languages IS DISTINCT FROM $10::text[] OR certifications IS DISTINCT FROM $11::text[] OR education IS DISTINCT FROM $12::jsonb OR work_history IS DISTINCT FROM $13::jsonb THEN 'pending' ELSE admin_approval_status END,verification_status=CASE WHEN qualifications IS DISTINCT FROM $9::text[] OR service_types IS DISTINCT FROM $8::text[] THEN 'pending' ELSE verification_status END WHERE user_id=$1 RETURNING *`,[req.user.id,b.headline,b.bio,b.experience_years,b.base_zip||null,b.service_radius_miles,b.service_zips,b.service_types,b.qualifications,b.languages,b.certifications,JSON.stringify(b.education),JSON.stringify(b.work_history),b.availability]);res.json({profile:rows[0]})}catch(e){next(e)}});
...
app.get("/api/requests/:id/matches",auth("customer"),member,async(req,res,next)=>{try{const request=await db().query("SELECT * FROM service_requests WHERE id=$1 AND customer_id=$2",[req.params.id,req.user.id]);if(!request.rowCount)throw fail(404,"Request not found.");const r=request.rows[0];if(r.clinical)throw fail(409,"Licensed care requests require credential and service-area review before matching. Contact MayBridge support.");const {rows}=await db().query(`SELECT u.id,u.display_name,p.headline,p.bio,p.experience_years,p.base_zip,p.service_radius_miles,p.service_zips,p.service_types,p.qualifications,p.languages,p.certifications,p.education,p.work_history,p.verification_status,p.background_check_status,p.admin_approval_status,p.rating,p.review_count,p.payments_enabled,p.availability,(SELECT count(*)::integer FROM bookings done WHERE done.provider_id=p.user_id AND done.status='completed') AS completed_services FROM provider_profiles p JOIN users u ON u.id=p.user_id WHERE u.disabled_at IS NULL AND p.verification_status='verified' AND p.background_check_status='clear' AND p.admin_approval_status='approved' AND p.credentials_expires_at>now() AND p.background_expires_at>now() AND $1=ANY(p.service_types) AND p.qualifications @> $2::text[] AND NOT EXISTS(SELECT 1 FROM bookings b WHERE b.provider_id=p.user_id AND b.status<>'canceled' AND tstzrange(b.starts_at,b.ends_at,'[)') && tstzrange($3::timestamptz,$3::timestamptz+($4||' minutes')::interval,'[)')) ORDER BY p.rating DESC NULLS LAST,u.display_name`,[r.service_type,r.required_qualifications,r.starts_at,r.duration_minutes]);const target=zipcodes.lookup(r.zip);const eligible=rows.filter(p=>{const exact=Array.isArray(p.service_zips)&&p.service_zips.includes(r.zip);const origin=zipcodes.lookup(p.base_zip);const distance=milesBetween(origin,target);p.distance_miles=Number.isFinite(distance)?Math.round(distance):null;return (exact||distance<=p.service_radius_miles)&&isAvailable(p.availability,r.starts_at,r.duration_minutes)});let amount_cents=null;try{amount_cents=serviceAmount(r.service_type,r.duration_minutes,process.env.SERVICE_HOURLY_RATES_JSON)}catch(e){if(e.status!==503)throw e}res.json({matches:eligible,amount_cents})}catch(e){next(e)}});
  ...
  try {
    ...
    const booking=await transaction(async c=>{
      ...
      if(existing){
        ...
        const stillEligible=await c.query("SELECT 1 FROM provider_profiles p JOIN users u ON u.id=p.user_id WHERE p.user_id=$1 AND u.disabled_at IS NULL AND p.verification_status='verified' AND p.background_check_status='clear' AND p.admin_approval_status='approved' AND p.credentials_expires_at>now() AND p.background_expires_at>now()",[existing.provider_id]);
        if(!stillEligible.rowCount)throw fail(409,"Provider is no longer eligible for booking. Contact support.");
        if(new Date(existing.starts_at)<=new Date())throw fail(409,"This service time has passed. Contact support.");
        return existing;
      }
      ...
      const provider=(await c.query("SELECT p.* FROM provider_profiles p JOIN users u ON u.id=p.user_id WHERE p.user_id=$1 AND u.disabled_at IS NULL AND $2=ANY(service_types) AND qualifications @> $3::text[] FOR UPDATE OF p",[req.body.provider_id,r.service_type,r.required_qualifications])).rows[0];
      const distance=milesBetween(zipcodes.lookup(provider?.base_zip),zipcodes.lookup(r.zip));
      const inArea=provider&&(provider.service_zips?.includes(r.zip)||distance<=provider.service_radius_miles);
      if(!provider || !inArea || provider.verification_status!=="verified" || provider.background_check_status!=="clear" || provider.admin_approval_status!=="approved" || !(new Date(provider.credentials_expires_at)>new Date()) || !(new Date(provider.background_expires_at)>new Date()))throw fail(400,"Provider is not eligible.");
    ...
    if(!(await syncConnect(db(),client,booking.stripe_account_id)).ready)throw fail(409,"This provider must finish payout setup before you can book.");
...
app.get("/api/admin/providers",auth("admin"),async(req,res,next)=>{try{const {rows}=await db().query(`SELECT u.id,u.display_name,u.email,p.* FROM provider_profiles p JOIN users u ON u.id=p.user_id ORDER BY CASE p.admin_approval_status WHEN 'pending' THEN 0 ELSE 1 END,u.display_name`);res.json({providers:rows})}catch(e){next(e)}});
app.put("/api/admin/providers/:id/review",auth("admin"),async(req,res,next)=>{try{
  const approval=String(req.body.admin_approval_status||""),background=String(req.body.background_check_status||""),verification=String(req.body.verification_status||"");
  if(!["pending","approved","rejected","suspended"].includes(approval)||!["not_started","pending","clear","consider","failed","expired"].includes(background)||!["not_started","pending","verified","rejected","expired"].includes(verification))throw fail(400,"Choose valid review statuses.");
  const reference=String(req.body.background_check_reference||"").trim().slice(0,250);
  const expires=key=>{const value=req.body[key];if(!value)return null;const date=new Date(value);if(!Number.isFinite(+date))throw fail(400,"Enter valid review dates.");return date.toISOString()};
  const credentialsExpiry=expires("credentials_expires_at"),backgroundExpiry=expires("background_expires_at");
  if(background==="clear"&&(!reference||!(new Date(backgroundExpiry)>new Date())))throw fail(400,"A clear screening result requires a vendor reference and future review date.");
  if(verification==="verified"&&!(new Date(credentialsExpiry)>new Date()))throw fail(400,"Verified credentials require a future expiry or review date.");
  if(approval==="approved"&&(background!=="clear"||verification!=="verified"))throw fail(400,"Complete screening and credential checks before approval.");
  const profile=await transaction(async c=>{
    const before=(await c.query("SELECT * FROM provider_profiles WHERE user_id=$1 FOR UPDATE",[req.params.id])).rows[0];if(!before)throw fail(404,"Provider not found.");
    const {rows}=await c.query(`UPDATE provider_profiles SET admin_approval_status=$2,background_check_status=$3,verification_status=$4,background_check_reference=$5,approved_at=CASE WHEN $2='approved' THEN now() ELSE NULL END,approved_by=CASE WHEN $2='approved' THEN $6::uuid ELSE NULL END,background_checked_at=CASE WHEN $3 IN ('clear','consider','failed') THEN now() ELSE background_checked_at END,credentials_expires_at=$7,background_expires_at=$8 WHERE user_id=$1 RETURNING *`,[req.params.id,approval,background,verification,reference||null,req.user.id,credentialsExpiry,backgroundExpiry]);
    const snapshot=p=>Object.fromEntries(["admin_approval_status","background_check_status","verification_status","background_check_reference","credentials_expires_at","background_expires_at"].map(k=>[k,p[k]]));
    await c.query("INSERT INTO provider_reviews_audit(provider_id,reviewer_id,before_state,after_state) VALUES($1,$2,$3,$4)",[req.params.id,req.user.id,JSON.stringify(snapshot(before)),JSON.stringify(snapshot(rows[0]))]);return rows[0];
  });res.json({profile});
}catch(e){next(e)}});

app.get(["/portal","/portal/"],(req,res)=>res.sendFile(path.join(path.dirname(fileURLToPath(import.meta.url)),"public/index.html")));
app.get(["/","/index.html","/mission.html","/values.html","/site.css","/maybridge-crest-transparent.png"],(req,res)=>res.sendFile(path.join(path.dirname(fileURLToPath(import.meta.url)),"..",req.path==="/"?"index.html":req.path.slice(1))));
...
app.use((err,req,res,next)=>{if(res.headersSent)return next(err);const status=err.status||(err.code==="22P02"?400:["23505","23P01"].includes(err.code)?409:500);if(status>=500)console.error(err.message);res.status(status).json({error:status>=500?"The service is temporarily unavailable.":err.message})});
